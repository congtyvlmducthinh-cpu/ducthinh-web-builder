...
function renderQuotationTab() {
  ...
  // == Khách hàng ==
  h += '<div class="quot-section"><div class="quot-section-title" onclick="toggleQuotSection(this)"><span class="badge blue">??</span><span class="title-text">Khách hàng</span><span class="quot-toggle-icon">?</span></div>';
  ...
  h += '<div class="quot-form-group"><label>?? Ngu?i liên h?</label><input type="text" id="qContact" placeholder="Ngu?i liên h?..." oninput="updateQuotPreview()" class="quot-input"></div></div>';
  h += '<div class="quot-form-row"><div class="quot-form-group"><label>?? Ngu?i ph? trách</label><input type="text" id="qAssigned" placeholder="Tên nhân viên..." oninput="updateQuotPreview()" class="quot-input"></div>';
  h += '<div class="quot-form-group"><label>?? Email KH</label><input type="email" id="qCustEmail" placeholder="Email khách hàng..." oninput="updateQuotPreview()" class="quot-input"></div></div></div></div>';
  ...
  h += '<div class="quot-form-row"><div class="quot-form-group"><label>EXW/FOB/CIF</label><select id="quotDeliveryMode" class="quot-input" onchange="onQuotDeliveryChange()"><option value="exw">EXW</option><option value="fob">FOB</option><option value="cif">CIF</option></select></div>';
  h += '<div class="quot-form-group" id="quotLccGroup" style="display:none"><label>LCC</label><select id="quotLccType" class="quot-input" onchange="recalcQuotCart()"><option value="no">No LCC</option><option value="sub">Sub LCC</option></select></div>';
  h += '<div class="quot-form-group" id="quotFreightGroup" style="display:none"><label>?? Cu?c (USD)</label><input type="number" id="quotFreight" class="quot-input" value="0" min="0" oninput="recalcQuotCart()" style="width:100px"></div></div>';
  h += '<div class="quot-form-row"><div class="quot-form-group"><label>?? Lo?i ti?n</label><select id="quotCurrency" class="quot-input" onchange="recalcQuotCart()"><option value="VND">VND</option><option value="USD">USD</option></select></div>';
  h += '<div class="quot-form-group"><label>?? Th? tru?ng</label><select id="quotMarket" class="quot-input" onchange="recalcQuotCart()"><option value="other">?? Khác</option><option value="cn">???? TQ</option></select></div>';
  h += '<div class="quot-form-group"><label>?? ÐVT</label><select id="quotUnit" class="quot-input" onchange="updateQuotPreview()"><option value="T?n">T?n</option><option value="MT">MT</option><option value="TON">TON</option></select></div></div>';
  h += '<div class="quot-form-row"><div class="quot-form-group"><label>? Hi?u l?c</label><select id="qValid" class="quot-input" onchange="updateQuotPreview()"><option value="15 ngày">15 ngày</option><option value="30 ngày">30 ngày</option><option value="7 ngày">7 ngày</option><option value="K? t? ngày ký">K? t? ngày ký</option></select></div>';
  h += '<div class="quot-form-group"><label>?? Thanh toán</label><select id="qPayment" class="quot-input" onchange="updateQuotPreview()"><option value="T/T 30 days">T/T 30 days</option><option value="T/T 60 days">T/T 60 days</option><option value="L/C at sight">L/C at sight</option><option value="Negotiable">Negotiable</option></select></div></div>';
  h += '<div class="quot-form-row"><div class="quot-form-group" id="quotPortGroup" style="display:none"><label>? C?ng di</label><input type="text" id="qPort" placeholder="C?ng di..." class="quot-input" oninput="updateQuotPreview()"></div>';
  h += '<div class="quot-form-group" id="quotDestGroup" style="display:none"><label>?? C?ng d?n</label><input type="text" id="qDest" placeholder="C?ng d?n..." class="quot-input" oninput="updateQuotPreview()"></div>';
  h += '<div class="quot-form-group"><label>?? Giao hàng</label><input type="text" id="qDelivery" placeholder="Ði?u ki?n giao hàng..." class="quot-input" oninput="updateQuotPreview()"></div></div>';
  h += '<div class="quot-form-group"><label>?? Ghi chú</label><input type="text" id="qNote" placeholder="Ghi chú..." class="quot-input" oninput="updateQuotPreview()" style="width:100%"></div></div></div>';

  // == S?n ph?m ==
  h += '<div class="quot-section"><div class="quot-section-title" onclick="toggleQuotSection(this)"><span class="badge purple">??</span><span class="title-text">S?n ph?m</span><span class="quot-toggle-icon">?</span>';
  h += '<button class="tckt-spec-btn" onclick="quotSpecPickerOpen()" title="Ch?n thông s? KT hi?n th?">??? Spec</button>';
  h += '<button class="quot-add-btn" onclick="quotAddRow()">+ Thêm SP</button></div>';
  h += '<div class="quot-section-content"><div id="quotCart"></div></div></div>';

  // == Action buttons ==
  h += '<div class="quot-actions" style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">';
  h += '<button class="quot-btn quot-btn-primary" onclick="quotPreviewOpen()">?? Xem báo giá</button>';
  h += '<button class="quot-btn quot-btn-secondary" onclick="quotPrint()">??? In</button>';
  h += '<button class="quot-btn quot-btn-secondary" onclick="quotCopy()">?? Copy</button>';
  h += '<button class="quot-btn quot-btn-secondary" onclick="quotClear()">??? Xóa h?t</button></div>';
  return h;
}
...
function quotPopulateFilters() { /* global filter removed */ if(!document.getElementById("quotMachFilter")) return;
  ...
  var curM=me.value;
  me.innerHTML='<option value="">— T?t c? máy —</option>';
  ...
  if(se) { se.innerHTML='<option value="">— T?t c? TC —</option>'; Object.keys(standards).sort().forEach(function(k){se.innerHTML+='<option value="'+k.replace(/"/g,"&quot;")+'">'+k+'</option>';}); }
  var sze=document.getElementById("quotSizeFilter");
  if(sze) { sze.innerHTML='<option value="">— T?t c? KT —</option>'; Object.keys(sizes).sort().forEach(function(k){sze.innerHTML+='<option value="'+k.replace(/"/g,"&quot;")+'">'+k+'</option>';}); }
}

// Filtered products helper
function getQuotFilteredProducts() {
  var mEl=document.getElementById("quotMachFilter");
  if(!mEl) return DATA_PRODUCTS.slice();
  var m=mEl.value;
  var sEl=document.getElementById("quotStdFilter");
  if(!sEl) return DATA_PRODUCTS.slice();
  var s=sEl.value;
  var szEl=document.getElementById("quotSizeFilter");
  if(!szEl) return DATA_PRODUCTS.slice();
  var sz=szEl.value;
  var list=[];
...
function quotFilterProds() {
  ...
  for(var idx=0;idx<QUOT_CART.length;idx++){
    ...
    sel.innerHTML='<option value="">— Ch?n SP —</option>';
    var lastMach="";
...
function quotRenderCart() {
  ...
  var h='<table class="quot-table">';
  h+='<thead><tr>';
  h+='<th class="quot-th-num">#</th>';
  h+='<th class="quot-th-prod">S?n ph?m <span style="font-weight:400;font-size:10px">(Máy/TC/KT)</span></th>';
  h+='<th class="quot-th-qty">SL</th>';
  h+='<th class="quot-th-bag">Bao bì</th>';
  h+='<th class="quot-th-special">QC d?c bi?t</th>';
  h+='<th class="quot-th-price">Giá bán</th>';
  h+='</tr></thead>';
  h+='<tbody>';
  ...
  cartEl.innerHTML=h;
  // Restore values
  for(var i=0;i<QUOT_CART.length;i++){var item=QUOT_CART[i];
    var machEl=document.getElementById("quotMach_"+i);var stdEl=document.getElementById("quotStd_"+i);var szEl=document.getElementById("quotSz_"+i);
    ...
    var psel=document.getElementById("quotProd_"+i);if(psel){for(var pi=0;pi<psel.options.length;pi++){if(psel.options[pi].value===item.product){psel.selectedIndex=pi;break;}}}
    var bagEl=document.getElementById("quotBagSpec_"+i);if(bagEl){for(var bi=0;bi<bagEl.options.length;bi++){if(bagEl.options[bi].value===item.bagSpec){bagEl.selectedIndex=bi;break;}}}
...
function quotRenderRow(idx, item) {
  ...
  for(var i=0;i<DATA_PRODUCTS.length;i++){var p=DATA_PRODUCTS[i];allMachines[p.machine]=1;allStds[p.standard]=1;allSizes[p.size]=1;}
  var availStds={},availSizes={};
  for(var i=0;i<DATA_PRODUCTS.length;i++){var p=DATA_PRODUCTS[i];var mOk=!curMach||String(p.machine)===curMach;var sOk=!curStd||p.standard===curStd;if(mOk)availStds[p.standard]=1;if(mOk&&sOk)availSizes[p.size]=1;}
  var machKeys=Object.keys(allMachines).sort();var stdKeys=Object.keys(availStds).sort();var sizeKeys=Object.keys(availSizes).sort();
  var bagSpecs={};for(var b=0;b<DATA_BAGS.length;b++){bagSpecs[DATA_BAGS[b].spec]=1;}var specKeys=Object.keys(bagSpecs).sort();

  var h='<tr class="quot-data-row" id="quotRow_'+idx+'">';

  // # + remove
  h+='<td class="quot-td-num">';
  h+='<span class="quot-row-num">#'+(idx+1)+'</span>';
  h+='<button class="quot-remove-btn" onclick="quotRemoveRow('+idx+')" title="Xóa">?</button>';
  h+='</td>';

  // SP + filters
  h+='<td class="quot-td-prod">';
  h+='<select id="quotProd_'+idx+'" class="quot-input quot-prod-sm" onchange="onQuotProdChange('+idx+')" style="width:100%"><option value="">— Ch?n SP —</option>';
  for(var i=0;i<DATA_PRODUCTS.length;i++){var p=DATA_PRODUCTS[i];var mOk=!curMach||String(p.machine)===curMach;var sOk=!curStd||p.standard===curStd;var zOk=!curSz||p.size===curSz;if(!mOk||!sOk||!zOk)continue;var val=p.code+'||'+(p.standard||'');var lab=p.code+' - '+p.size+(p.standard?' ('+p.standard+')':'');h+='<option value="'+val.replace(/"/g,"&quot;")+'"'+(item.product===val?' selected':'')+'>'+escHtml(lab)+'</option>';}
  h+='</select>';
  h+='<div class="quot-fltr-row">';
  h+='<select id="quotMach_'+idx+'" class="quot-fltr-sm" onchange="onQuotRowFilterChange('+idx+')"><option value="">Máy</option>';
  for(var i=0;i<machKeys.length;i++){h+='<option value="'+machKeys[i].replace(/"/g,"&quot;")+'"'+(curMach===machKeys[i]?' selected':'')+'>'+machKeys[i]+'</option>';}
  h+='</select>';
  h+='<select id="quotStd_'+idx+'" class="quot-fltr-sm" onchange="onQuotRowFilterChange('+idx+')"><option value="">TC</option>';
  for(var i=0;i<stdKeys.length;i++){h+='<option value="'+stdKeys[i].replace(/"/g,"&quot;")+'"'+(curStd===stdKeys[i]?' selected':'')+'>'+stdKeys[i]+'</option>';}
  h+='</select>';
  h+='<select id="quotSz_'+idx+'" class="quot-fltr-sm" onchange="onQuotRowFilterChange('+idx+')"><option value="">KT</option>';
  ...
  // SL
  h+='<td class="quot-td-qty"><input type="number" id="quotQty_'+idx+'" class="quot-input" placeholder="SL" value="'+item.qty+'" min="1" oninput="onQuotQtyChange('+idx+')" style="width:55px"></td>';

  // Bao bì
  h+='<td class="quot-td-bag">';
  h+='<select id="quotBagSpec_'+idx+'" class="quot-input quot-prod-sm" onchange="onQuotBagSpecChange('+idx+')" style="width:100%">';
  for(var b=0;b<specKeys.length;b++){h+='<option value="'+specKeys[b].replace(/"/g,"&quot;")+'"'+(item.bagSpec===specKeys[b]?' selected':'')+'>'+specKeys[b]+'</option>';}
  h+='<option value="Khác">Khác</option></select>';
  h+='<div id="quotBagDetail_'+idx+'" style="margin-top:3px"><select id="quotBag_'+idx+'" class="quot-input quot-prod-sm" onchange="recalcQuotCart()" style="width:100%"><option value="">Bao</option></select></div>';
  h+='</td>';

  // QC d?c bi?t
  h+='<td class="quot-td-special">';
  var jumboDisp=item.bagSpec==="Jumbo"?"":"none";
  var otherDisp=item.bagSpec!=="Jumbo"?"":"none";
  h+='<div id="quotJumboRow_'+idx+'" style="display:'+jumboDisp+'">';
  h+='<select id="quotJumboTons_'+idx+'" class="quot-input quot-prod-sm" onchange="onQuotJumboTonsChange('+idx+')" style="width:100%">';
  var jumboTons=["0.5","0.7","1","1.1","1.2","1.25","1.3","1.35","1.375","1.38","1.4","1.5"];
  h+='<option value="1">Jumbo (t?n)</option>';
  for(var t=0;t<jumboTons.length;t++){h+='<option value="'+jumboTons[t]+'"'+(String(item.rowJumboTons)===jumboTons[t]?' selected':'')+'>'+jumboTons[t]+' t?n</option>';}
  h+='</select></div>';
  h+='<div id="quotOtherRow_'+idx+'" style="display:'+otherDisp+'">';
  h+='<select id="quotOther_'+idx+'" class="quot-input quot-prod-sm" onchange="onQuotOtherChange('+idx+')" style="width:100%"><option value="">QC khác</option>';
  for(var o=0;o<DATA_OTHERS.length;o++){h+='<option value="'+DATA_OTHERS[o].code.replace(/"/g,"&quot;")+'"'+(item.rowOtherCode===DATA_OTHERS[o].code?' selected':'')+'>'+escHtml(DATA_OTHERS[o].code)+'</option>';}
  h+='</select></div>';
  h+='<div id="quotOtherTonsRow_'+idx+'" style="display:'+otherDisp+';margin-top:2px">';
  h+='<select id="quotOtherTons_'+idx+'" class="quot-input quot-prod-sm" onchange="recalcQuotCart()">';
  var otherTons=["0.5","0.7","1","1.1","1.2","1.25","1.3","1.35","1.4","1.5"];
  h+='<option value="1">1 t?n</option>';
  for(var t=0;t<otherTons.length;t++){h+='<option value="'+otherTons[t]+'"'+(String(item.rowOtherTons)===otherTons[t]?' selected':'')+'>'+otherTons[t]+' t?n</option>';}
  h+='</select></div>';
  h+='</td>';

  // Giá bán
  h+='<td class="quot-td-price"><input type="number" id="quotSell_'+idx+'" class="quot-input quot-sell-sm" placeholder="Giá..." min="0" step="0.01" value="'+(item.sellPrice||'')+'" oninput="onQuotSellChange('+idx+')" style="width:110px"></td>';

  h+='</tr>';

  // Sub-row: TCKT / Min / HH
  h+='<tr class="quot-subrow" id="quotSubRow_'+idx+'">';
  h+='<td colspan="6" class="quot-td-stats">';
  h+='<div class="quot-row-stats">';
  h+='<span class="quot-spec-row" id="quotSpec_'+idx+'">?? TCKT: <span class="quot-spec-val">Ch?n SP tru?c</span></span>';
  h+='<span class="quot-min-price" id="quotMin_'+idx+'">?? Min: <span class="quot-min-val">—</span></span>';
  h+='<span class="quot-comm-display" id="quotComm_'+idx+'">?? HH: <span class="quot-comm-val">—</span></span>';
  h+='</div>';
  h+='</td></tr>';
  return h;
...
function onQuotBagSpecChange(idx) {
  ...
  var html="";
  if(val==="Khác"){
    html='<label>QC khác</label><input type="text" id="quotBag_'+idx+'" class="quot-input" placeholder="Nh?p quy cách..." oninput="recalcQuotCart()">';
  } else {
    var bags=[], j=0;
    for(j=0;j<DATA_BAGS.length;j++){
      if(DATA_BAGS[j].spec===val){bags.push(DATA_BAGS[j]);}
    }
    html='<label>Bao</label><select id="quotBag_'+idx+'" class="quot-input" onchange="recalcQuotCart()"><option value="">— M?c d?nh —</option>';
  ...
  if(!val){if(specEl)specEl.querySelector(".quot-spec-val").textContent="Ch?n SP tru?c";recalcQuotCart();return;}
  ...
  if(specEl&&prod){
    ...
    if(s){
      ...
      specEl.querySelector(".quot-spec-val").textContent="(Không có thông s? TCKT)";
    }
  }
  recalcQuotCart();
}
...
function onQuotQtyChange(idx) {
  var el=document.getElementById("quotQty_"+idx); if(!el) return;
  QUOT_CART[idx].qty=Math.max(1, parseInt(el.value)||1);
  recalcQuotCart();
}
function onQuotDeliveryChange() {
  ...
  // EXW: hide c?ng di, FOB: show c?ng di, CIF: show c?ng di + c?ng d?n
  var portGroup=document.getElementById("quotPortGroup");
  var destGroup=document.getElementById("quotDestGroup");
  if(portGroup) portGroup.style.display=(mode==="exw"?"none":"block");
  if(destGroup) destGroup.style.display=(mode==="cif"?"block":"none");
  recalcQuotCart();
...
function recalcQuotCart() {
  ...
  for(var i=0;i<QUOT_CART.length;i++){
    ...
    var qtyEl=document.getElementById("quotQty_"+i); if(qtyEl) item.qty=Math.max(1, parseInt(qtyEl.value)||1);
    ...
    if(prod && sellPrice > 0 && totalCost > 0){
      ...
      commissionVar = diff * 0.2;
      totalComm = effCommBase + commissionVar;
    }

    if(minPriceEl){
      minPriceEl.innerHTML='?? Min: <span class="quot-min-val">'+fmtNum(minPrice,isUsd)+' '+cc+'</span>';
    }
    if(commEl){
      var h='?? HH: <span class="quot-comm-val">'+fmtNum(totalComm,isUsd)+' '+cc+'</span>';
      h+='<div style="font-size:10px;color:var(--muted);margin-top:2px">';
      if(effCommBase>0) h+='Base: '+fmtNum(effCommBase,isUsd)+' | ';
      h+='20% chênh: '+fmtNum(commissionVar,isUsd)+' '+cc+'</div>';
      commEl.innerHTML=h;
    }
  }
  updateQuotPreview();
...
function _generateQuotPreviewHtml() {
  ...
  if(!hasProd) return '<div class="quot-preview-empty" style="padding:40px;text-align:center;color:#94a3b8">Chua có s?n ph?m nào. Hãy thêm s?n ph?m tru?c.</div>';
  ...
  var unit=document.getElementById("quotUnit")?document.getElementById("quotUnit").value:"T?n";
  ...
  if(port) h+='<span><strong>C?ng di:</strong> '+port+'</span>';
  h+='</div>';
  h+='<table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:12px">';
  h+='<thead><tr style="background:#f1f5f9"><th style="padding:6px 8px;border:1px solid #e2e8f0;text-align:left;font-weight:600;color:#475569">STT</th><th style="padding:6px 8px;border:1px solid #e2e8f0;text-align:left;font-weight:600;color:#475569">Code</th><th style="padding:6px 8px;border:1px solid #e2e8f0;text-align:left;font-weight:600;color:#475569">S?n ph?m</th><th style="padding:6px 8px;border:1px solid #e2e8f0;text-align:left;font-weight:600;color:#475569">Bao bì</th><th style="padding:6px 8px;border:1px solid #e2e8f0;text-align:left;font-weight:600;color:#475569">ÐVT</th><th style="padding:6px 8px;border:1px solid #e2e8f0;text-align:left;font-weight:600;color:#475569">SL</th><th style="padding:6px 8px;border:1px solid #e2e8f0;text-align:right;font-weight:600;color:#475569">Giá bán</th><th style="padding:6px 8px;border:1px solid #e2e8f0;text-align:left;font-weight:600;color:#475569">Ti?n t?</th></tr></thead>';
  h+='<tbody>';
  for(var i=0;i<QUOT_CART.length;i++){
    var item=QUOT_CART[i];
  ...
  h+='<span><strong>Ði?u ki?n:</strong> '+mode+'</span><br>';
  if(port&&(mode==="FOB"||mode==="CIF")) h+='<span><strong>C?ng di:</strong> '+port+'</span><br>';
  var dest=document.getElementById("qDest")?document.getElementById("qDest").value:"";
  if(dest&&mode==="CIF") h+='<span><strong>C?ng d?n:</strong> '+dest+'</span><br>';
  h+='<span><strong>Hi?u l?c:</strong> '+valid+'</span><br>';
  ...
  h+='- Giá không bao g?m thu? VAT<br>';
  h+='- Báo giá này có hi?u l?c trong vòng '+valid+'<br>';
  if(mode==='FOB'||mode==='CIF') h+='- S? lu?ng t?i thi?u: 3 containers';
  h+='</div></div>';
  return h;
}
function updateQuotPreview() {
  ...
  for(var i=0;i<QUOT_CART.length;i++){if(QUOT_CART[i].product){hasProd=true;break;}}
  if(!hasProd){
    previewEl.innerHTML='<div class="quot-preview-empty">?? Ch?n s?n ph?m d? xem tru?c báo giá</div>';
    return;
  }
  ...
  if(contact) h+='<div class="q-row"><strong>Ngu?i liên h?:</strong><span>'+contact+'</span></div>';
  if(assigned) h+='<div class="q-row"><strong>Ngu?i ph? trách:</strong><span>'+assigned+'</span></div>';
  ...
  if(port) h+='<div class="q-row"><strong>C?ng di:</strong><span>'+port+'</span></div>';

  // Product table
  h+='<table class="q-table">';
  h+='<thead><tr><th>STT</th><th>Code</th><th>S?n ph?m</th><th>Spec</th><th>Bao</th><th>ÐVT</th><th>SL</th><th>Giá bán</th><th>Ti?n t?</th></tr></thead>';
  h+='<tbody>';
  for(var i=0;i<QUOT_CART.length;i++){
    var item=QUOT_CART[i];
    if(!item.product) continue;
  ...
  h+='<div class="q-row"><strong>Ði?u ki?n:</strong><span>'+mode+'</span></div>';
  if(port&&(mode==="FOB"||mode==="CIF")) h+='<div class="q-row"><strong>C?ng di:</strong><span>'+port+'</span></div>';
  var dest=document.getElementById("qDest")?document.getElementById("qDest").value:"";
  if(dest&&mode==="CIF") h+='<div class="q-row"><strong>C?ng d?n:</strong><span>'+dest+'</span></div>';
  h+='<div class="q-row"><strong>Hi?u l?c:</strong><span>'+valid+'</span></div>';
...
function quotCopy() {
  var txt = quotGetText();
  if (!txt) { alert("Chua có d? li?u báo giá!"); return; }
  if (navigator.clipboard) {
    navigator.clipboard.writeText(txt).then(function(){alert("? Ðã copy báo giá vào clipboard");});
  } else {
    prompt("Copy do?n text du?i dây:", txt);
  }
}
...
function quotGetText() {
  ...
  if (contact) t += "Ngu?i liên h?: " + contact + "\n";
  if (assigned) t += "Ngu?i ph? trách: " + assigned + "\n";
  if (custEmail) t += "Email: " + custEmail + "\n";
  t += "Ngày: " + dateStr + "\n";
  if (port) t += "C?ng di: " + port + "\n";
  var dest=document.getElementById("qDest")?document.getElementById("qDest").value:"";
  if (dest&&mode==="CIF") t += "C?ng d?n: " + dest + "\n";
  t += "----------------------------------\n";
  for (var i = 0; i < QUOT_CART.length; i++) {
    var item = QUOT_CART[i];
    if (!item.product) continue;
  ...
  t += "----------------------------------\n";
  t += "- Giá không bao g?m thu? VAT\n";
  t += "- Báo giá này có hi?u l?c " + valid + "\n";
  return t;
}
...
function quotPreviewOpen() {
  ...
  if(!overlay){
    ...
    var closeBtn=document.createElement("button");closeBtn.textContent="? Ðóng";
...
function quotPrint() {
  var hasProd = false;
  for (var i = 0; i < QUOT_CART.length; i++) { if (QUOT_CART[i].product) { hasProd = true; break; } }
  if (!hasProd) { alert("Chua có s?n ph?m d? in báo giá!"); return; }
  var customer = document.getElementById("qCustomer").value || "______________________";
  var contact = document.getElementById("qContact").value || "";
  var assigned = document.getElementById("qAssigned").value || "";
  var custEmail = document.getElementById("qCustEmail").value || "";
  ...
  w.document.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>Báo giá - Ð?c Th?nh</title>');
  ...
  if (contact) w.document.write('<div class="row"><strong>Ngu?i liên h?:</strong><span>'+contact+'</span></div>');
  if (assigned) w.document.write('<div class="row"><strong>Ngu?i ph? trách:</strong><span>'+assigned+'</span></div>');
  ...
  if (port) w.document.write('<div class="row"><strong>C?ng di:</strong><span>'+port+'</span></div>');
  var dest=document.getElementById("qDest")?document.getElementById("qDest").value:"";
  if (dest&&mode==="CIF") w.document.write('<div class="row"><strong>C?ng d?n:</strong><span>'+dest+'</span></div>');
  w.document.write('<table><thead><tr><th>STT</th><th>Code</th><th>S?n ph?m</th><th>Spec</th><th>Bao</th><th>ÐVT</th><th>SL</th><th>Giá bán</th><th>Ti?n t?</th></tr></thead><tbody>');
  for (var i = 0; i < QUOT_CART.length; i++) {
    var item = QUOT_CART[i];
    if (!item.product) continue;
    var parts = item.product.split("||"), code = parts[0], std = parts[1] || "";
  ...
  w.document.write('<div class="row"><strong>Ði?u ki?n:</strong><span>'+mode+'</span></div>');
  w.document.write('<div class="row"><strong>Hi?u l?c:</strong><span>'+valid+'</span></div>');
  ...
  w.document.write('<div class="footer">- Giá không bao g?m thu? VAT<br>- Báo giá này có hi?u l?c trong vòng '+valid+'<br>' + (mode==='FOB'||mode==='CIF' ? '- S? lu?ng t?i thi?u: 3 containers<br><br>' : '<br>') + '<strong>'+COMPANY_EN+'</strong></div>');
...
var QUOT_TCKT_FIELDS = [
  ...
  { key:"destiny", label:"T? tr?ng" },
  { key:"hardness", label:"Ð? c?ng" },
...
function renderSpecFields(specData) {
  if (!specData) return "(Không có thông s? TCKT)";
  ...
  return parts.length > 0 ? parts.join(" | ") : "(Không có thông s? TCKT)";
}
...
function quotSpecPickerOpen() {
  ...
  if (!overlay) {
    ...
    h += '<div class="tckt-header"><h3>??? Ch?n thông s? k? thu?t</h3><button class="tckt-close" onclick="quotSpecPickerClose()">?</button></div>';
    ...
    h += '<option value="">— Ch?n template —</option>';
    ...
    h += '<button class="tckt-btn-sm tckt-btn-primary" onclick="quotSpecPickerSave()">?? Luu</button>';
    h += '<button class="tckt-btn-sm tckt-btn-danger" onclick="quotSpecPickerDelete()">???</button>';
    h += '</div>';

    h += '<div class="tckt-footer">';
    h += '<button class="tckt-btn-sm tckt-btn-secondary" onclick="quotSpecPickerClose()">Hu?</button>';
    h += '<button class="tckt-btn-sm tckt-btn-primary" onclick="quotSpecPickerApply()">? Áp d?ng</button>';
    h += '</div>';
    h += '</div></div>';
    ...
    if (tplSel) {
      ...
      tplSel.innerHTML = '<option value="">— Ch?n template —</option>';
...
function quotSpecPickerSave() {
  ...
  if (!name) { alert("Vui lòng nh?p tên template!"); return; }
...
function quotSpecPickerApply() {
  ...
  if (keys.length === 0) {
    if (!confirm("B?n chua ch?n thông s? nào. Ti?p t?c?")) return;
  }
  quotSetSelectedFields(keys);
  // Auto-save as "M?c d?nh" template so reload remembers
  var tpls = quotGetTemplates();
  tpls["M?c d?nh"] = keys.slice();
  quotSetTemplates(tpls);
  quotSetLastTemplate("M?c d?nh");
  quotSpecPickerClose();
  ...
  for (var i2 = 0; i2 < QUOT_CART.length; i2++) {
    ...
    if (prod) {
      ...
      if (specEl) {
        var s = DATA_SPECS[prod.code];
        specEl.querySelector(".quot-spec-val").textContent = s ? renderSpecFields(s) : "(Không có thông s? TCKT)";
        changed = true;
      }
    }
  }
...
function toggleQuotSection(titleEl) {
  var section = titleEl.closest(".quot-section");
  if (!section) return;
  section.classList.toggle("collapsed");
  var icon = titleEl.querySelector(".quot-toggle-icon");

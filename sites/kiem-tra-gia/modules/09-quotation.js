...
function getCalcPriceData() {
  ...
  var prod=null;
  for (var i=0;i<DATA_PRODUCTS.length;i++) { var _qp=psel.value.split('||'),_qc=_qp[0],_qs=_qp[1]||'';if (DATA_PRODUCTS[i].code===_qc&&DATA_PRODUCTS[i].standard===_qs) { prod=DATA_PRODUCTS[i]; break; } }
  if (!prod) return null;

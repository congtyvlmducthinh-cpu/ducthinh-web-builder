...
# Find the upload handler - the part where DATA_PRODUCTS = s
old = '''      if (prodSheet) {
...
new = '''      if (prodSheet) {
        var s = XLSX.utils.sheet_to_json(wb.Sheets[prodSheet]);
        if (s.length > 0) {
          // Fill market variants from base prices
...
print('Updated upload handler to auto-fill market fields!')

...
function renderCalcTab() {
  ...
  h += '<div class="calc-section-header"><span class="calc-section-icon">📦</span><span>Chọn sản phẩm</span></div>';
  ...
  h += '<div class="calc-row-group">';
  h += '<div class="calc-row-half"><label>🏭 Máy</label>';
  ...
  h += '</select></div>';
  h += '<div class="calc-row-half"><label>📋 Tiêu chuẩn</label>';
  ...
  // --- Section: Bao bì ---
  h += '<div class="calc-section">';
  h += '<div class="calc-section-header"><span class="calc-section-icon">🛍️</span><span>Tùy chọn bao bì</span></div>';

  // Bag spec + Bag select side by side
  h += '<div class="calc-row-group">';
  h += '<div class="calc-row-half"><label>📏 Quy cách bao</label>';
  h += '<select id="calcBagSpec" onchange="filterBagSpec()"><option value="25KG">25KG</option><option value="34KG">34KG</option><option value="50KG">50KG</option><option value="Jumbo">Jumbo</option></select></div>';
  h += '<div class="calc-row-half"><label>🛍️ Loại bao</label>';
  h += '<select id="calcBag" onchange="calcPrice()"><option value="">— Không chọn —</option>';
  h += '</select></div>';
  h += '</div>';

  // Jumbo tonnage (hidden by default)
  h += '<div class="calc-row" id="calcTonnageRow" style="display:none">';
  h += '<label>⚖️ Số tấn / bao Jumbo</label>';
  h += '<select id="calcTonnage" onchange="calcPrice()">';
  ...
  h += '<div class="calc-section">';
  h += '<div class="calc-section-header"><span class="calc-section-icon">📦</span><span>Quy cách khác</span></div>';

  h += '<div class="calc-row-group">';
  h += '<div class="calc-row-half"><label>📋 Loại quy cách</label>';
  ...
  h += '</select></div>';
  h += '<div class="calc-row-half" id="calcOtherTonnageRow">';
  h += '<label>⚖️ Số tấn</label>';
  ...
  h += '<span class="calc-right-icon">💰</span>';
...
function calcCommission() {
  ...
  var h = '<div class="calc-comm-row"><span>Hoa hồng cơ bản</span><strong>' + fmtNum(commBase, isUsd) + ' ' + (isUsd ? "USD" : "VND") + '</strong></div>';
...
var total = 0;
  ...
  h += '<div class="calc-result-item"><span class="calc-rl">💰 ' + modeLabel + ' ' + (hasBag ? 'chưa bao bì' : 'có bao bì') + '</span><span class="calc-rv exw">' + fmtNum(exwMin, isUsd) + ' ' + cc + '</span></div>';
...
function handleManageFile(file) {
  ...
  reader.onload = function(e) {
    try {
      ...
      var prodSheet = hasSheet(["Products", "products", "Sản phẩm", "sản phẩm", "Product", "product"]);
      if (prodSheet) {
      ...
      var fobSheet = hasSheet(["CostFOB", "Cost FOB", "costfob", "cost fob", "FOB", "fob"]);
      if (fobSheet) {

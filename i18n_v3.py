#!/usr/bin/env python3
"""
Multi-language injector v3 - safe single-pass approach
"""
import re, json
...
# Step 1: Inject LANG after CDN script
...
# Step 3: Find protected regions (LANG object + DATA arrays)
lang_start = original.find('// ===== ĐA NGÔN NGỮ =====')
lang_end = original.find('</script>', lang_start) + 9
print(f'Protected: LANG obj at {lang_start}-{lang_end}')
...
for m in re.finditer(r'var (DATA_PRODUCTS|DATA_BAGS|DATA_OTHERS|DATA_MAX_LOADING)\s*=\s*\[', original):
    ...
    print(f'Protected: DATA at {s}-{pos}')

# Step 4: Single-pass sentinel replacement
sentinel_map = {}
result = []
i = 0
...
while i < len(original):
    ...
    # Try to match a key (longest first)
    matched = False
    for key in sorted_keys:
        if original[i:i+len(key)] == key:
            # Generate sentinel for this key
            sid = None
...
# Step 6: Clean up some edge cases
# Empty calls:  + __('') +  -> remove
...
count = text.count("__(")
nest = text.count("'+ __(") + text.count('"+ __("')
print(f'Total __() calls: {count}')
print(f'Possible missed protected areas: {nest}')
print('Done!')
